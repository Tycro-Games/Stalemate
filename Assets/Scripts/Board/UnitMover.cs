...
public class UnitMover : MonoBehaviour
{
    ...
    public IEnumerator PreviewMovementUnits(List<UnitRenderer> init, List<UnitRenderer> fin, bool isRed)
    {
        ...
        for (int i = 0; i < fin.Count; i++)
        {
            ...
            {
                List<UnitRenderer> intermediateUnits = Board.GetUnitRenderersBetween(init[i], fin[i], ref pieces);
    ...
    {
        GameObject intermediateExp = Instantiate(sprite, unit.transform.position, Quaternion.identity, transform);
    ...
    private IEnumerator MoveToTarget(Transform toMoveTransform, Transform target)
    {
        ...
        //  moves a thing from point A to B
