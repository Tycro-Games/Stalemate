...
public class Timeline : MonoBehaviour
{
    ...
    [SerializeField]
    private UnityEvent onPresentTurn;
    [SerializeField]
    private UnityEvent onTimeChange;
    ...
    public void GetCurrentPieces()
    {
        ...
        foreach (var unit in currentBoard)
            prevBoard.Add(unit.Clone());
    ...
    public void StepAhead()
    {
        ...
        for (var i = 0; i < prevBoard.Count; i++)
            board.pieces[i].SetUnitData(prevBoard[i]);
        unitManager.ResetRedBlueUnitLists();
        onTimeChange?.Invoke();
        if (index == previosBoard.Count - 1)
            onPresentTurn.Invoke();
    }
