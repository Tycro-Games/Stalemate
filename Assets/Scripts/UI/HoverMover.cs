...
public class HoverMover : MonoBehaviour {
  ...
  public void MoveClamped(Vector3 pos) {
    ...
    // Calculate the snapped position by rounding to the nearest squareSize multiple
    float snappedX = Mathf.Round(pos.x / squareSize) * squareSize;
    float snappedY = Mathf.Round(pos.y / squareSize) * squareSize;

    // Clamp the snapped position within the limits
  ...
  private void OnDrawGizmosSelected() {
    Gizmos.DrawWireCube(transform.position + (Vector3)((minLimits + maxLimits) / 2.0f),
                        (maxLimits));
  }
}
