...
public class UnitPlacer : MonoBehaviour {
  [SerializeField]
  private bool placeOnOppositeSide = false;
  [SerializeField]
  ...
  [SerializeField]
  private Vector3Event onHover;
  [SerializeField]
  private UnitInfoEvent onHoverUnitInfo;
  ...
  [SerializeField]
  private UnityEvent onNoHover;
  [SerializeField]
  private UnityEvent onHoverNewTile;
  [SerializeField]
  private UnityEvent onValidTurn;
  [SerializeField]
  private UnityEvent onNotValidTurn;

  private CellFinder cellFinder;
  private CursorController cursorController;
  [SerializeField]
  private int maxPlacements = 4;
  ...
  private void CheckRemainingPoints() {
    if (RedBlueTurn.currentPoints < unitSettings.unitSettings.cost)
      Reset();
  }
  ...
  private void PlaceUnit(InputAction.CallbackContext context) {
    ...
    if (context.control.IsPressed()) {
      var selectedPiece = place.transform.GetChild(0).GetComponent<UnitRenderer>();
      selectedPiece.SetUnitSettingsAndHp(unitSettings);
  ...
  public void Place(Vector3 input) {
    ...
    if (selectedRenderer.sprite != null)
      onHoverUnitInfo?.Invoke(
          place.transform.GetChild(0).GetComponent<UnitRenderer>().GetUnitSettings());
    else
      onHoverNewTile?.Invoke();
