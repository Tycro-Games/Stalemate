...
public class Spawning
{
    ...
    {
        placement = new List<int>(4) { [0] = spawning.placement[0], [1] = spawning.placement[1],
                                       [2] = spawning.placement[2], [3] = spawning.placement[3] };
    }
}
...
{
    [SerializeField]
    private UnityEvent onFogOfWar;
    [SerializeField]
    private UnityEvent onEnemyEndTurn;
    [SerializeField]
    private UnitRenderEvent onEnemyPlace;
    private Board board;
    private Peak peak;
    [SerializeField]
    private UnitBoardInfo fogOfWarRed;
    [SerializeField]
    private UnitBoardInfo fogOfWarBlue;
    [SerializeField]
    private Transform redUnitsParent;
    [SerializeField]
    private Transform blueUnitsParent;
    [SerializeField]
    private ScriptableAI redAI;
    [SerializeField]
    private ScriptableAI blueAI;
    ...
    public void Init()
    {
        ...
        foreach (var blue in blues)
            blueUnits.Add(blue.GetBoardInfo());
    ...
    private void ChooseEnemies()
    {
        ...
        else
        {
            ...
            if (choiceIndex == -1)
                Debug.Log("Choice index for option is invalid");
    ...
    public void DebugChooseMaximalSpawning()
    {
        ...
        for (var i = 0; i < spawnsScore.Count; i++)
            if (spawnsScore[i].Item2 == highestScore)
                ...
                break; // Since spawnsScore is sorted, no need to check further once a
    ...
    public void ClearPositions()
    {
        ...
        foreach (var t in positions)
            t.SetUnitSettings(new UnitBoardInfo());
    }

    private void GenerateSpawnings()
    {
        maxSlots = positions.Count - 1;
    ...
    private bool RandomSpawn(bool isRed)
    {
        ...
        var fogOfWar = isRed ? fogOfWarBlue : fogOfWarRed;
        foreach (var unitRenderer in positions)
            unitRenderer.SetUnitSettings(fogOfWar);
    ...
    public void ChooseSpawningRate(float index)
    {
        ...
        foreach (var unitSlot in positions)
        {
            if (placement.placement[enemyIdx] > 0)
                SetUnitRenderer(unitSlot, enemyList[placement.placement[enemyIdx] - 1]);
    ...
    private void SetUnitRendererAndInitializeHealth(UnitRenderer unitRenderer, UnitBoardInfo settings)
    {
    ...
    {
        var temp =
            enemies.Where(en => en.unitSettings.cost <= maxWeight && en.unitSettings.cost >= minWeight).ToArray();
    ...
    public void RateBoard()
    {
        ...
        {
            var blueScore = blue.Count * pointSystem.alliedUnits;
            var redScore = red.Count * pointSystem.enemyUnits;

            var blueOverLineScore = blueOverLine * pointSystem.alliedUnitsOverLine;
            var redOverLineScore = redOverLine * pointSystem.enemyUnitsOverLine;
