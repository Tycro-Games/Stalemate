...
public class AIPlacer : MonoBehaviour {
  ...
  public void Init() {
    ...
    foreach (var red in reds) redUnits.Add(red.GetBoardInfo());
  ...
  private void ChooseEnemies() {
    ...
    // this AI chooses stuff
      ...
      positions =
          new List<UnitRenderer>(board.GetEmptySquares(isRed ? SquareType.BLUE : SquareType.RED));
  ...
  public void ChooseMaximalSpawning(ref int index) {
    ...
    var highestScore = spawnsScore.First().Item2 -
                       topScores;  // Get the highest score and substract the first ones
  ...
  public void ClearPositions() {
    ...
    foreach (var t in positions) t.SetUnitSettings(new UnitBoardInfo());
  }
  ...
  private void DisplaySpawning(Spawning toDisplay) {
    // Debug.Log(toDisplay.placement[0] + " " + toDisplay.placement[1] + " " +
    // toDisplay.placement[2] + " " +
    //           toDisplay.placement[3]);
  }

  private string MakeString(Spawning toDisplay) {
    return "[" + toDisplay.placement[0] + ", " + toDisplay.placement[1] + ", " +
           toDisplay.placement[2] + ", " + toDisplay.placement[3] + "]";
  }
    ...
    for (var i = 0; i <= 5; i++) {
      ...
      if (k == emptySlots) {
        if (IsValid(spawning.placement)) {
          ...
          validSpawns.Add(
              new Spawning(newSpawning[0], newSpawning[1], newSpawning[2], newSpawning[3]));
          spawningsCount++;
  ...
  private UnitBoardInfo ChooseEnemy(int minWeight, int maxWeight, List<UnitBoardInfo> enemies) {
    var temp =
        enemies.Where(en => en.unitSettings.cost <= maxWeight && en.unitSettings.cost >= minWeight)
            .ToArray();
  ...
  public void RateBoard() {
    ...
    if (!isRed) {
      ...
      var blueOverLineScore =
          blueOverLine * pointSystem.alliedUnitsOverLine;                   // Blue units over line
